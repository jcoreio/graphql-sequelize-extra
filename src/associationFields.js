...
import { defaultArgs, resolver } from '@jcoreio/graphql-sequelize'
...
type Options = {|
  ...
  getResolver?: <T: Model<any>>(
    model: Class<T>,
    association: Association<T, Model<any>>
  ) => Function,
|}
...
function associationFields<TSource, TContext>(
  ...
  const getResolver =
    options.getResolver || ((model, association) => resolver(association))
  ...
  for (let key in model.associations) {
    ...
    if (!resolve)
      throw new Error(
        `missing resolver for model: ${String(target)}, association: ${String(
          association
        )}`
      )
